...
from transformers import CLIPVisionModel, AutoTokenizer, CLIPFeatureExtractor
...
class Predictor:
    def __init__(self, model_weights_path, mapping_type, clip_length, num_layers, training_model=None, prefix_length=40, prefix_size=640):
    ...
    def predict(self, image, use_beam_search=True):
        """Run a single prediction on the model"""
    ...
    def test(self, test_path, image_start_path, use_beam_search=True, output_name=""):
...
def generate2(
    ...
    generated_list = []
    stop_token_index = 3#tokenizer.encode(stop_token)[0]
    print("stop token: ", stop_token_index)
